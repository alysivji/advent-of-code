...
const isReportSafe = (report: number[]) => {
  ...
  if (diff.map(level => Math.abs(level)).filter((level) => level < 1 || level > 3).length > 0) {
    return false
  }
...
const findSafeReports = (reports: number[][]) => {
  return reports.map(report => isReportSafe(report)).filter(value => value === true).length
}
...
assert(findSafeReportsWithProblemDampener(testReports) === 4, "part 2 test input failed")
