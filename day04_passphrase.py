...
PROBLEM_INPUT = """nyot babgr babgr kqtu kqtu kzshonp ylyk psqk
...
def valid_phrase(passphrase: str, anagram=False) -> bool:
    ...
    for i, word in enumerate(phrases):
        for another_word in phrases[(i+1):]:
            if len(word) == len(another_word):
                if Counter(word) == Counter(another_word):
                    return False
    return True


assert valid_phrase("aa bb cc dd ee") is True
assert valid_phrase("aa bb cc dd aa") is False
assert valid_phrase("aa bb cc dd aaa") is True


def check_system_passphrases(system_passphrases: str, anagram=False) -> int:
    phrases = system_passphrases.split('\n')
    total_valid = 0
    for passphrase in phrases:
        if valid_phrase(passphrase, anagram) is True:
            total_valid += 1
    return total_valid


assert valid_phrase("abcde fghij", anagram=True) is True
assert valid_phrase("abcde xyz ecdab", anagram=True) is False
assert valid_phrase("a ab abc abd abf abj", anagram=True) is True
assert valid_phrase("iiii oiii ooii oooi oooo", anagram=True) is True
assert valid_phrase("oiii ioii iioi iiio ", anagram=True) is False


if __name__ == "__main__":
    print(check_system_passphrases(PROBLEM_INPUT))
    print(check_system_passphrases(PROBLEM_INPUT, anagram=True))
